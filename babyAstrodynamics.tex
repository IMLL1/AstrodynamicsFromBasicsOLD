...
\begin{figure}[H]
    ...
    \def\dV{1.25}
    \definecolor{purpA}{rgb}{.4,0,.8}
    \definecolor{purpB}{rgb}{.8,0,.4}
    \begin{tikzpicture}[>=latex]
        \draw[thin, dotted, blue] (0,0) ellipse ({\Rone} and {\Rone});
        \draw[blue, thick,>-] (-\Rone,0) arc (180:360:{\Rone} and {\Rone}) node[right] {$r_1$};

        \draw[thin, dotted, purpA] (\ctrXa,0) ellipse ({\trSMAa} and {\trSmAa});
        \draw[purpA, thick] (\Rone,0) arc (0:180:{\trSMAa} and {\trSmAa});

        \draw[thin, dotted, purpB] (\ctrXb,0) ellipse ({\trSMAb} and {\trSmAb});
        \draw[purpB, thick] (-\trAP,0) arc (180:360:{\trSMAb} and {\trSmAb});

        \draw[thin, dotted,red] (0,0) ellipse ({\Rtwo} and {\Rtwo});
        \draw[red, thick,->] (\Rtwo,0) arc (0:180:{\Rtwo} and {\Rtwo}) node[right] {$r_2$};;
